...
def realizar_estudio_unificado_por_navegador(archivos_dict):
    ...
    for nav in navegadores_unicos:
        print(f"--- PROCESANDO SIMILITUD DE HUELLAS DE CL POR {nav.upper()} Y POR WEB ---")
        ...
        for i in range(len(records)):
            for j in range(i + 1, len(records)):
                ...
                # Filtro: Misma Web (dentro del mismo Navegador)
        ...
        if res:
            ...
            nombre_fichero = f"DIST_HUELLAS_CL_NAV_{nav.upper()}.csv"
            ...
            print("-" * 40)
            print(f" RESULTADOS FINALES: {nav.upper()}")
            print("-" * 40)
...
if __name__ == "__main__":
    ...
    realizar_estudio_unificado_por_navegador(mis_archivos)