...
def realizar_estudio_ja4s_por_navegador(archivos_dict):
    ...
    navegadores_unicos = df_total['navegador'].unique()

    for nav in navegadores_unicos:
        print(f"--- PROCESANDO SIMILITUD JA4S (SERVIDOR) POR {nav.upper()} Y POR WEB ---")
        
        # Filtramos por el navegador del cliente para ver la respuesta del servidor
        df_nav = df_total[df_total['navegador'] == nav].copy()
        records = df_nav.to_dict('records')
        res = []
        ...
        for i in range(len(records)):
            for j in range(i + 1, len(records)):
                ...
                # Filtro: Misma Web (dentro del mismo Navegador del cliente)
                if r1['web_buscada'] == r2['web_buscada']:
                    ...
                    # 1. JA4S_a: Levenshtein (Estructura: Protocolo, TLS, ALPN)
                    a1, a2 = p1[0], p2[0]
                    dist_lev_a = Levenshtein.distance(a1, a2)
                    sim_lev_a = 1 / (1 + dist_lev_a)

                    # 2. JA4S_b: Cipher Suite elegido (Comparación binaria)
                    b1, b2 = p1[1], p2[1]
                    sim_cipher_b = 1.0 if b1 == b2 else 0.0

                    # 3. JA4S_c: Hash de Extensiones (Comparación binaria)
                    c1, c2 = p1[2], p2[2]
        ...
        if res:
            ...
            nombre_fichero = f"DIST_JA4S_SRV_NAV_{nav.upper()}.csv"
...
if __name__ == "__main__":
    ...
    realizar_estudio_ja4s_por_navegador(mis_archivos)